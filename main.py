...
async def lifespan(app: FastAPI):
    # Load the ML model once at startup rather than per-request.
    # If the file doesn't exist, model_loader sets predictions_enabled=False
    # and the /v1/teams/next-game endpoint omits prediction fields silently.
...
app = FastAPI(title="CFB ATS API", version="0.0.1", lifespan=lifespan)
...
SAMPLE_PREDICITONS = [
    GamePrediction(game_id=1234, home_team_id=333, away_team_id=194, spread=-7.5, prob_cover=0.62, model_pick=True),
    GamePrediction(game_id=5678, home_team_id=251, away_team_id=333, spread=+3.0, prob_cover=0.41, model_pick=False),
]


@app.get("/health")
def health():
    return {"ok": True, "version": "0.0.1", "predictions_enabled": model_loader.predictions_enabled}
